...
def _recall_eval(predicted, reference, weights=None, micro=False):
    ...
    global current_set_neg, current_set_pos
    ...
    for i in range(len(wlist)):
        ...
        for ref in refA:
            ...
            if tot == 0:
                ...
                maxtot = 0
                continue
            for (mindex, pre) in enumerate(preA):
                try:
                ...
                if maxtot == -1 or r > maxr:
                    maxr = r
                    maxtot = tot
                    seg = predicted.mstrs[w][mindex]
        if maxr == 1.0:
            #current_set_pos.append('{}\t{}\n'.format(w, seg))
            current_set_pos.append('{}\n'.format(w))
        else:
            #current_set_neg.append('{}\t{}\n'.format(w, seg))
            current_set_neg.append('{}\n'.format(w))
        if micro:
            # Micro-average: 
...
not_underseg = []
not_overseg = []
current_set_pos = None
current_set_neg = None
...
def run_evaluation(options):
    ...
    if options.tokens:
        ...
        global underseg, overseg, perfect, current_set_pos, current_set_neg
        current_set_neg = underseg
        current_set_pos = not_underseg
        rec, recn = _recall_eval(predicted, goldstd, weights=weights, 
                                 micro=options.micro)
        current_set_neg = overseg
        current_set_pos = not_overseg
        pre, pren = _recall_eval(goldstd, predicted, weights=weights, 
                                 micro=options.micro)

    with open('{}_underseg'.format(options.name), 'w') as f:
        for seg in underseg:
            f.write(seg)
    with open('{}_overseg'.format(options.name), 'w') as f:
        for seg in overseg:
            f.write(seg)
    with open('{}_not_underseg'.format(options.name), 'w') as f:
        for seg in not_underseg:
            f.write(seg)
    with open('{}_not_overseg'.format(options.name), 'w') as f:
        for seg in not_overseg:
            f.write(seg)
...
def main(argv):
    ...
    parser.add_option("-n", "--name", dest="name",
                      default = None,
                      help="name of data set, for output file names")
